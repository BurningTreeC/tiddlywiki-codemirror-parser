/*\
title: $:/plugins/BurningTreeC/tiddlywiki-codemirror/plugins/lint.js
...
function getKnownFilterOperators() {
	...
	commonOps.forEach(function(op) { operators.add(op); });
...
function findUnmatchedBrackets(text, openSeq, closeSeq, startPos) {
	...
	while (i < text.length) {
		if (text.substring(i, i + openLen) === openSeq) {
			stack.push({ pos: startPos + i, char: openSeq });
			i += openLen;
		} else if (text.substring(i, i + closeLen) === closeSeq) {
			if (stack.length > 0) {
				stack.pop();
			} else {
				unmatched.push({ pos: startPos + i, char: closeSeq, type: "unexpected" });
			}
			i += closeLen;
		} else {
			i++;
	...
	stack.forEach(function(item) {
		unmatched.push({ pos: item.pos, char: item.char, type: "unclosed" });
	});
...
function checkFilterBrackets(text, startPos) {
	...
	while (i < text.length) {
		...
		if (char === '[') {
			...
			if (squareStack.length > 0) {
				...
				issues.push({ pos: startPos + i, message: "Unexpected ']' in filter" });
			}
		} else if (char === '{') {
	...
	squareStack.forEach(function(pos) {
		issues.push({ pos: pos, message: "Unclosed '[' in filter" });
	});
	curlyStack.forEach(function(pos) {
		issues.push({ pos: pos, message: "Unclosed '{' in filter text reference" });
	});
	angleStack.forEach(function(pos) {
		issues.push({ pos: pos, message: "Unclosed '<' in filter variable" });
	});
...
function findBestInsertPosition(state, afterPos, containerLimit, allTagPositions) {
	...
	for (var lineNum = lineAfter.number + 1; lineNum <= state.doc.lines; lineNum++) {
		...
		if (line.text.trim() === "") {
			...
			return { pos: line.from, type: "emptyLine" };
		}
	}
		...
		if (closeMatch) {
			...
			if (!endName && pragmaStack.length > 0) {
				...
				if (pragmaStack.length > 1) {
					...
					var openNames = pragmaStack.map(function(p) { return p.name; }).join(", ");
					issues.push({
						from: endPos,
						to: endPos + endLength,
	...
	for (var i = 0; i < lines.length; i++) {
		...
		if (trimmed.startsWith("```")) {
			if (stack.length > 0 && trimmed === "```") {
				...
				stack.push({ pos: pos + line.indexOf("```"), lineNum: i, lineEnd: pos + line.length });
			}
		}
		...
		if (trimmed.startsWith("$$$")) {
			if (stack.length > 0 && stack[stack.length - 1].type === "$$$" && trimmed === "$$$") {
				...
				stack.push({ pos: pos + line.indexOf("$$$"), lineNum: i, lineEnd: pos + line.length, type: "$$$" });
			}
		}
			...
			if (tagName) {
				(function(name, tag, pos, type, html) {
					...
					actions.push({
						name: actionName,
						apply: function(view, from, to) {
							view.dispatch({ changes: { from: pos, to: pos, insert: tag + "\n" } });
						}
					});
			...
			if ((isAmbiguous || isBareEndHint) && issue.suggestedName) {
				...
				(function(name) {
					actions.push({
						name: "Use \\end " + name,
						apply: function(view, from, to) {
							view.dispatch({ changes: { from: to, to: to, insert: " " + name } });
						}
					});
				})(issue.suggestedName);
			}
				...
				(function(targetPos, name) {
					if (typeof targetPos === "number") {
						...
						actions.push({
							...
							apply: function(view, from, to) {
								view.dispatch({ changes: { from: targetPos, to: targetPos, insert: "\\end " + name + "\n" } });
							}
						});
					} else {
						...
						actions.push({
							...
							apply: function(view, from, to) {
								...
								view.dispatch({ changes: { from: docLength, to: docLength, insert: "\n\\end " + name + "\n" } });
							}
						});
					}
					actions.push({
						...
						apply: function(view, from, to) {
							...
							view.dispatch({ changes: { from: pos, to: pos, insert: prefix + "\\end\n" } });
						}
					});
				})(insertAt, pragmaName);
			}
			...
			if (typeof insertAt === "number") {
				...
				(function(pos, type) {
					...
					actions.push({
						name: actionName,
						apply: function(view, from, to) {
							view.dispatch({ changes: { from: pos, to: pos, insert: "<%endif%>\n" } });
						}
					});
				})(insertAt, insertType);
			}
		...
		codeIssues.forEach(function(issue) {
			...
			(function(m, pos, type) {
				...
				actions.push({
					name: actionName,
					apply: function(view, from, to) {
						view.dispatch({ changes: { from: pos, to: pos, insert: m + "\n" } });
					}
				});
