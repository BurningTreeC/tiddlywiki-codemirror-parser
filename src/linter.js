 ...
 */
export const ACTION_IMPLICIT_VARIABLES = {
 ...
 */
function getWidgetName(widgetNode, doc) {
 ...
 */
function getAttributeName(attrNode, doc) {
 ...
 */
export function matchesWildcardPattern(varName, pattern) {
    if (pattern.endsWith("*")) {
 ...
 */
export function getActionContextVariables(node, doc) {
    const result = {
        variables: [],
        patterns: []
    };
 ...
 */
export function isActionContextVariable(varName, context) {
 ...
 */
function extractParameterNamesFromNode(defNode, doc) {
 ...
 */
function extractParameterNames(defNode, doc) {
    const params = new Set();
 ...
 */
function validateSubstitutedParams(defNode, validParams, doc, diagnostics) {
                ...
                if (paramNameNode.name === "SubstitutedParamName") {
                    ...
                    if (!validParams.has(paramName) && !builtInVariables.has(paramName)) {
                        diagnostics.push({
                            ...
                            to: node.to,
                            severity: "error",
                            message: `Unknown parameter "${paramName}". Available parameters: ${validParams.size > 0 ? Array.from(validParams).join(", ") : "(none)"}`,
                        });
                    }
                    break;
                }
 ...
 */
function findOrphanedSubstitutedParams(tree, defRanges, doc, diagnostics) {
    const stack = [];
    let node = tree.firstChild;
    while (node) {
        if (node.name === "SubstitutedParam") {
            // Check if this node is inside any definition
            const isInDefinition = defRanges.some((range) => node.from >= range.from && node.to <= range.to);
            if (!isInDefinition) {
 ...
 * Skips nested definitions (they'll be handled separately with their own parameters).
 */
function validatePlaceholders(defNode, validParams, doc, diagnostics) {
    const stack = [];
    let node = defNode.firstChild;
    while (node) {
        // Skip nested definitions - they'll be handled separately with inherited params
 ...
 */
function findOrphanedPlaceholders(tree, macroDefRanges, doc, diagnostics) {
    const stack = [];
    let node = tree.firstChild;
    while (node) {
        if (node.name === "Placeholder") {
            // Check if this node is inside a macro definition (\define only)
            const isInMacroDef = macroDefRanges.some((range) => node.from >= range.from && node.to <= range.to);
            if (!isInMacroDef) {
                // Find the param name for the error message
                let paramName = "?";
                let paramNameNode = node.firstChild;
 ...
 */
function warnSubstitutedParamsInWrongContext(defNode, defType, doc, diagnostics) {
    const stack = [];
    let node = defNode.firstChild;
 ...
 */
function lintParameters(view) {
    const diagnostics = [];
    const tree = syntaxTree(view.state);
    const doc = view.state.doc.toString();
    // Track definition ranges for orphan detection
    const substitutionDefRanges = [];
    const macroDefRanges = [];
