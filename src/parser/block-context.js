...
export class BlockContext {
            ...
            if (trimmed.startsWith("<!--")) {
                ...
                if (endIdx !== -1) {
                    ...
                    if (afterComment.trim()) {
                        ...
                        const paragraphElt = this.elt(Type.Paragraph, commentEndPos, commentEndPos + afterComment.length, inlineElements);
    ...
    looksLikeBlockStart(text) {
        ...
        if (ch === Ch.Exclamation)
            return true;
        // List items * # ; :
        if (ch === Ch.Asterisk || ch === Ch.Hash || ch === Ch.Semicolon || ch === Ch.Colon)
            return true;
        // Block quote <<<
        if (text.startsWith("<<<"))
            return true;
    ...
    hasUpcomingPragma() {
        ...
        while (this.nextLine()) {
            ...
            if (text.startsWith("```")) {
                ...
                while (this.nextLine()) {
                    if (this._line.text.trim().startsWith("```"))
                        break;
                }
                continue;
            }
     ...
     */
    isBlockStarter(text, firstChar, hasLeadingWhitespace) {
        ...
        if (firstChar === Ch.LessThan) {
            ...
            if (text.startsWith("<<")) {
                ...
                if (closeIdx === -1)
                    return true; // No closing >> - multi-line macro block
                // Has closing >> - check if there's content after (then it's inline, not block)
                if (text.slice(closeIdx + 2).trim())
                    return false;
            ...
            if (!tagMatch)
                return false; // Not a recognizable tag pattern
        ...
        if (firstChar === Ch.Dollar && text.startsWith("$$") && !text.startsWith("$$$")) {
            if (this.parsers.some(p => p.name === "KaTeXBlock"))
                return true;
        }
        return false;
    }
     ...
     */
    parseInlineRange(from, to) {
        if (from >= to)
            return [];
        const content = this.input.read(from, to);
        return this.parser.parseInline(content, from);
    }
    /**
