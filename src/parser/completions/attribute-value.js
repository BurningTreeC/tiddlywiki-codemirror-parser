...
import { defaultFieldNames, buildMultiSelectionChanges, extractLocalDefinitions, getTiddlerBoost, getTiddlerSortText, triggerCompletionEffect, } from "./common";
...
let _commonMacrosSet = null;
function getCommonMacrosSet() {
    if (!_commonMacrosSet) {
 ...
 */
function findCurrentTagStart(text) {
 ...
 * Attribute value completion source
 */
export function attributeValueCompletion(getMacroNames, getTiddlerTitles, getFunctionNames, getVariableNames, getFieldNames, getTiddlerIndexes, getStoryViews, getDeserializers, getCSSValues, isDraftTiddler, getClassNames, getCSSValuesForProperty) {
    return (context) => {
        ...
        if (attrName.startsWith("style.")) {
            ...
            if (cssValues && cssValues.length > 0) {
                ...
                const filtered = cssValues.filter(value => value.toLowerCase().startsWith(lowerPartial));
        ...
        // $variable attribute
            ...
            const lowerPartial = partial.toLowerCase();
            const filtered = allNames.filter(({ name }) => name.toLowerCase().startsWith(lowerPartial));
        ...
        // $message or message attribute - only for widgets that support it
            ...
            if (showMessageCompletions) {
                const lowerPartial = partial.toLowerCase();
                const filtered = tiddlyWikiMessages.filter(msg => msg.toLowerCase().startsWith(lowerPartial));
        ...
        // Supports multiple space-separated classes
            ...
            if (getClassNames) {
                ...
                if (classNames && classNames.length > 0) {
                    ...
                    const filtered = classNames.filter(cls => cls.toLowerCase().startsWith(lowerCurrentWord) && !existingClasses.has(cls));
 ...
 */
function getWikitextAttributeContext(textBefore) {
 ...
 */
function findEnclosingPragmaInContent(content, posInContent) {
 ...
 */
function extractLocalDefsFromContent(content) {
    const macros = [];
    const procedures = [];
    const functions = [];
    const widgets = [];
...
export function wikitextAttributeCompletion(getTiddlerTitles, getMacroNames, getWidgetNames, getFunctionNames, getVariableNames, isDraftTiddler, getMacroParams) {
    ...
    return (context) => {
        ...
        if (!attrContext || !attrContext.isWikitext)
            return null;
        ...
        if (macroSubstMatch && enclosingPragma && enclosingPragma.params.length > 0) {
            ...
            const filtered = enclosingPragma.params.filter(p => p.toLowerCase().startsWith(partial.toLowerCase()));
        ...
        if (varSubstMatch && enclosingPragma && enclosingPragma.type === "define") {
            ...
            for (const p of enclosingPragma.params)
                seen.add(p);
            const vars = [];
            for (const name of contentDefs.procedures) {
                if (!seen.has(name)) {
                    seen.add(name);
                    vars.push({ name, detail: "procedure" });
                }
            }
            for (const name of contentDefs.functions) {
                if (!seen.has(name)) {
                    seen.add(name);
                    vars.push({ name, detail: "function" });
                }
            }
            for (const name of contentDefs.macros) {
                if (!seen.has(name)) {
                    seen.add(name);
                    vars.push({ name, detail: "macro" });
                }
            }
            for (const name of contentDefs.widgetVars) {
                if (!seen.has(name)) {
                    seen.add(name);
                    vars.push({ name, detail: "variable" });
                }
            }
            ...
            const filtered = vars.filter(v => v.name.toLowerCase().startsWith(partial.toLowerCase()));
        ...
        if (macroCallMatch) {
            ...
            if (!inQuote) {
                ...
                if (paramMatch) {
                    ...
                    if (params.length > 0) {
                        const filtered = params.filter(p => p.toLowerCase().startsWith(partial.toLowerCase()));
                        for (const param of filtered) {
                            options.push({
                                label: param,
                                type: "property",
        ...
        if (filterVarMatch && !content.endsWith('<$') && !content.endsWith('<<')) {
            ...
            for (const name of contentDefs.macros) {
                if (!seen.has(name)) {
                    seen.add(name);
                    allVars.push({ name, detail: "macro", boost: 2 });
                }
            }
            for (const name of contentDefs.procedures) {
                if (!seen.has(name)) {
                    seen.add(name);
                    allVars.push({ name, detail: "procedure", boost: 2 });
                }
            }
            for (const name of contentDefs.functions) {
                if (!seen.has(name)) {
                    seen.add(name);
                    allVars.push({ name, detail: "function", boost: 2 });
                }
            }
            for (const name of contentDefs.widgetVars) {
                if (!seen.has(name)) {
                    seen.add(name);
                    allVars.push({ name, detail: "variable", boost: 2 });
                }
            }
            ...
            for (const name of localDefs.macros) {
                if (!seen.has(name)) {
                    seen.add(name);
                    allVars.push({ name, detail: "macro", boost: 1 });
                }
            }
            for (const name of localDefs.procedures) {
                if (!seen.has(name)) {
                    seen.add(name);
                    allVars.push({ name, detail: "procedure", boost: 1 });
                }
            }
            for (const name of localDefs.functions) {
                if (!seen.has(name)) {
                    seen.add(name);
                    allVars.push({ name, detail: "function", boost: 1 });
                }
            }
            ...
            if (getMacroNames) {
                for (const name of getMacroNames()) {
                    if (!seen.has(name)) {
                        seen.add(name);
                        allVars.push({ name, detail: "macro", boost: 0 });
                    }
                }
            }
            if (getFunctionNames) {
                for (const name of getFunctionNames()) {
                    if (!seen.has(name)) {
                        seen.add(name);
                        allVars.push({ name, detail: "function", boost: 0 });
                    }
                }
            }
            if (getVariableNames) {
                for (const name of getVariableNames()) {
                    if (!seen.has(name)) {
                        seen.add(name);
                        allVars.push({ name, detail: "variable", boost: 0 });
                    }
                }
            }
            const filtered = allVars.filter(v => v.name.toLowerCase().startsWith(partial.toLowerCase()));
            for (const v of filtered) {
                options.push({
                    label: prefix + v.name,
        ...
        if (htmlMatch && !content.endsWith('<$') && !content.endsWith('<<')) {
            ...
            const filtered = commonHtmlTags.filter(t => t.toLowerCase().startsWith(partial.toLowerCase()));
        ...
        if (macroMatch) {
            ...
            if (enclosingPragma && enclosingPragma.params.length > 0) {
                const filteredParams = enclosingPragma.params.filter(p => p.toLowerCase().startsWith(partial.toLowerCase()));
            ...
            const docText = state.doc.toString();
            const localDefs = extractLocalDefinitions(docText);
            for (const m of localDefs.macros)
                allMacros.add(m);
            for (const m of localDefs.procedures)
                allMacros.add(m);
            for (const m of localDefs.functions)
                allMacros.add(m);
            // Also add from content definitions (pragmas defined within the attribute)
            for (const m of contentDefs.macros)
                allMacros.add(m);
            for (const m of contentDefs.procedures)
                allMacros.add(m);
            for (const m of contentDefs.functions)
                allMacros.add(m);
            for (const m of contentDefs.widgetVars)
                allMacros.add(m);
            if (getMacroNames) {
                for (const m of getMacroNames())
                    allMacros.add(m);
            }
            if (getFunctionNames) {
                for (const m of getFunctionNames())
                    allMacros.add(m);
            }
            if (getVariableNames) {
                for (const m of getVariableNames())
                    allMacros.add(m);
            }
            const filtered = Array.from(allMacros).filter(m => m.toLowerCase().startsWith(partial.toLowerCase()));
        ...
        if (linkMatch) {
            ...
            const titles = getTiddlerTitles ? getTiddlerTitles() : [];
            const filtered = titles.filter(t => t.toLowerCase().startsWith(partial.toLowerCase()));
        ...
        if (conditionalMatch) {
            ...
            const filtered = conditionals.filter(c => c.toLowerCase().startsWith(partial.toLowerCase()));
        ...
        if (pragmaMatch) {
            ...
            const filtered = pragmaKeywords.filter(kw => kw.label.toLowerCase().startsWith(partial.toLowerCase()));
        ...
        if (rulesMatch) {
            ...
            const filtered = rulesKeywords.filter(kw => kw.label.toLowerCase().startsWith(partial.toLowerCase()));
            for (const kw of filtered) {
                options.push({
                    label: kw.label,
                    type: "keyword",
        ...
        if (whitespaceMatch) {
            ...
            const filtered = whitespaceKeywords.filter(kw => kw.label.toLowerCase().startsWith(partial.toLowerCase()));
            for (const kw of filtered) {
                options.push({
                    label: kw.label,
                    type: "keyword",
        ...
        if (parsermodeMatch) {
            ...
            const filtered = parsermodeKeywords.filter(kw => kw.label.toLowerCase().startsWith(partial.toLowerCase()));
            for (const kw of filtered) {
                options.push({
                    label: kw.label,
                    type: "keyword",
