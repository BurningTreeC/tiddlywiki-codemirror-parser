 ...
 */
function isClosingLine(doc, lineNumber, marker) {
    let openCount = 0;
 ...
 */
export function fencedCodeCompletion(codeLanguages) {
    return (context) => {
 ...
 */
export function typedBlockCompletion(getTypeNames, getFileExtensions) {
    return (context) => {
        const pos = context.pos;
        const line = context.state.doc.lineAt(pos);
        const textBefore = context.state.doc.sliceString(line.from, pos);
