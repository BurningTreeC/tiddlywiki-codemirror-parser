 ...
 */
function isInsideConditionalContext(state, pos) {
...
export const filterOperatorMeta = {
 ...
 */
export function filterOperatorCompletion(getFilterOperators, getFilterBracketMode, getFunctionParams) {
    return (context) => {
...
function createFilterOperatorResult(context, partial, _matchLength, inFilterContext, getFilterOperators, getFilterBracketMode, getFunctionParams) {
    ...
    localFunctions.forEach(fn => {
        if (addedOps.has(fn.toLowerCase()))
            return;
    ...
    if (getFunctionParams) {
        const externalFunctions = customOperators.filter(op => !coreFilterOperators.includes(op) && !addedOps.has(op.toLowerCase()));
 ...
 */
export function filterRunPrefixCompletion(context) {
    const { state, pos } = context;
    const textBefore = state.sliceDoc(Math.max(0, pos - 100), pos);
 ...
 */
export function filterOperatorSuffixCompletion(getFieldNames) {
    return (context) => {
        const { state, pos } = context;
        const textBefore = state.sliceDoc(Math.max(0, pos - 100), pos);
        ...
        if (!suffixPart || !suffixPart.includes(':'))
            return null;
 ...
 */
export function filterOperandValueCompletion(getTiddlerNames, getTagNames, getFieldNames, getFunctionNames, getVariableNames, getTypeNames) {
    return (context) => {
        const { state, pos } = context;
        const textBefore = state.sliceDoc(Math.max(0, pos - 100), pos);
        ...
        if (meta.dynamicOperands) {
            ...
            options.push(...values
                ...
                .map(v => ({
                label: v,
                type: meta.dynamicOperands === 'fields' ? 'variable' :
                    meta.dynamicOperands === 'functions' ? 'function' :
                        'text',
                detail: detailType,
 ...
 */
export function imageLinkCompletion() {
    return (context) => {
        const { state, pos } = context;
        const textBefore = state.sliceDoc(Math.max(0, pos - 100), pos);
