 ...
 */
function getActionVariablesFromCallSites(defName, tree, doc, defFrom, defTo) {
    const result = {
        variables: [],
        patterns: []
    };
    ...
    function getAttributeName(attrNode) {
    ...
    while ((match = macroCallRegex.exec(doc)) !== null) {
        ...
        if (callPos >= defFrom && callPos <= defTo)
            continue;
 ...
 */
function getWidgetVariablesFromCallSites(defName, doc, defFrom, defTo) {
    const result = [];
    const seen = new Set();
    // Helper to extract variables from widget text using regex
    // This is simpler than tree traversal and works for our needs
    function extractVariablesFromEnclosingWidgets(callPos) {
 ...
 */
export function macroCompletion(getMacroNames, getFunctionNames, getVariableNames) {
    return (context) => {
        ...
        if (enclosingPragma && !hasDirectActionContext) {
            const callSiteActionVars = getActionVariablesFromCallSites(enclosingPragma.name, syntaxTree(state).topNode, docText, enclosingPragma.from, enclosingPragma.to);
        ...
        if (filterVarMatch) {
            ...
            const options = allNames.map(({ name, detail, type, boost }) => ({
                label: prefix + name,
                type,
                detail,
                boost,
 ...
 */
export function macroParamCompletion(getMacroParams) {
    return (context) => {
        const { state, pos } = context;
 ...
 */
function findEnclosingPragma(text, pos) {
...
export function substitutedParamCompletion() {
    return (context) => {
        ...
        if (syntaxType === "macroSubst") {
            ...
            const makeVarsubstCompletion = (name, detail, boost) => ({
                label: "$(" + name + ")$",
                type: "variable",
                detail,
                boost,
            ...
            const callSiteVars = getWidgetVariablesFromCallSites(enclosingPragma.name, docText, enclosingPragma.from, enclosingPragma.to);
