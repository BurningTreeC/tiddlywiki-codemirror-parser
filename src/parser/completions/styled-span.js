 ...
 */
function getStyleAttributeContent(context, getWidgetAttributes) {
 ...
 */
function getStyledSpanContent(context) {
    const tree = syntaxTree(context.state);
    let node = tree.resolveInner(context.pos, -1);
 ...
 */
export function styledSpanClassCompletion(getPageClasses) {
    return function (context) {
        ...
        if (!pageClasses || pageClasses.length === 0)
            return null;
        // Filter classes that match the prefix (case-insensitive)
        const lowerPartial = partial.toLowerCase();
        const matchingClasses = pageClasses.filter(cls => cls.toLowerCase().startsWith(lowerPartial));
        if (matchingClasses.length === 0)
            return null;
        const options = matchingClasses.map(cls => ({
            label: cls,
            type: "class",
            detail: "CSS class"
        }));
 ...
 */
export function styledSpanPropertyCompletion(getCSSProperties, getWidgetAttributes) {
    return function (context) {
        if (!getCSSProperties)
            return null;
        ...
        if (isStyledSpan && /\.[a-zA-Z_-]*$/.test(content))
            return null;
        ...
        const lowerPartial = partial.toLowerCase();
        const matchingProps = cssProperties.filter(prop => prop.toLowerCase().startsWith(lowerPartial));
...
//# sourceMappingURL=styled-span.js.map