 ...
 */
export function tiddlerCompletion(getTiddlerTitles, getImageTiddlerTitles, isDraftTiddler) {
    return (context) => {
        ...
        const inFilterContext = (/\{\{\{[^}]*$/.test(textBefore) ||
        ...
        if (match === filterTextRefMatch) {
            ...
            const options = titles.map(title => ({
                ...
                apply: (view, _completion, from, to) => {
                    ...
                    if (hasClosingBrace)
                        suffix = "";
                    if (!hasClosingBrace && !hasOuterBracket)
                        suffix = "}]";
 ...
 */
export function systemTiddlerCompletion(getTiddlerTitles, isDraftTiddler) {
    return (context) => {
        const { state, pos } = context;
        ...
        if (systemTitles.length === 0)
            return null;
        const partial = systemMatch[0];
        const from = pos - partial.length;
        const options = systemTitles.map(title => ({
            label: title,
            type: "variable",
 ...
 */
export function transclusionFieldCompletion(getTiddlerFields, getTiddlerIndexes) {
    return (context) => {
        const { state, pos } = context;
        const textBefore = state.sliceDoc(Math.max(0, pos - 200), pos);
        const transcludeFieldMatch = /(?<!\{)\{\{([^{}|]*?)!![^{}|!]*$/.exec(textBefore);
        const transcludeIndexMatch = /(?<!\{)\{\{([^{}|]*?)##[^{}|#]*$/.exec(textBefore);
        let textRefFieldMatch = null;
        let textRefIndexMatch = null;
        const inFilterContext = (/\[[^\]]*\{[^{}]*$/.test(textBefore) ||
            /\{\{\{[^}]*\{[^{}]*$/.test(textBefore) ||
            /\bfilter\s*=\s*(?:"[^"]*|'[^']*|"""[^"]*)\{[^{}]*$/.test(textBefore));
        if (inFilterContext) {
        ...
        const filtered = values.filter(v => v.toLowerCase().startsWith(partialValue.toLowerCase()));
        if (filtered.length === 0)
            return null;
        const patternLen = match[0].length;
        const closingSuffix = isTransclusion ? "}}" : "}";
        const options = filtered.map(value => ({
            label: prefix + value,
            type: "property",
            detail,
            apply: (view, _completion, from, to) => {
...
//# sourceMappingURL=tiddler.js.map