...
import { selfClosingWidgets, triggerCompletionEffect, buildMultiSelectionChanges, extractLocalDefinitions, } from "./common";
...
let _messagesSet = null;
function getMessagesSet() {
    if (!_messagesSet) {
...
function isSelfClosing(name, dynamicSelfClosing) {
    if (selfClosingWidgets.has(name))
        return true;
 ...
 */
export function widgetCompletion(getWidgetNames, getSelfClosingWidgets) {
    return (context) => {
        ...
        const options = allWidgets.map(({ name, detail }) => {
            const selfClose = isSelfClosing(name, getSelfClosingWidgets);
            return {
                label: "<" + name,
                type: "keyword",
 ...
 */
function findOpenWidgetTag(text) {
    ...
    while (pos < len) {
        ...
        if (ch === '`') {
            if (afterName.slice(pos, pos + 3) === '```') {
                pos += 3;
                while (pos < len && afterName.slice(pos, pos + 3) !== '```')
                    pos++;
                pos += 3;
            }
            else {
                pos++;
                while (pos < len && afterName[pos] !== '`')
                    pos++;
                pos++;
            }
            continue;
...
export function widgetAttributeCompletion(getMacroParams, getWidgetAttributes, getCSSProperties) {
    return (context) => {
        const { state, pos } = context;
        ...
        if (inQuote || filterDepth > 0 || macroDepth > 0)
            return null;
        ...
        if (partial.startsWith("style.") && getCSSProperties) {
            ...
            const matchingProps = cssProps.filter(prop => prop.toLowerCase().startsWith(lowerPartial));
            if (matchingProps.length === 0)
                return null;
            const options = matchingProps.map(prop => ({
                label: "style." + prop,
                type: "property",
                detail: "CSS property",
                apply: (view, _completion, from, to) => {
        ...
        const options = allAttrs.map(attr => ({
            label: attr,
            type: "property",
 ...
 */
export function closingTagCompletion(getWidgetNames) {
    return (context) => {
        const { state, pos } = context;
        ...
        if (!widgetCloseMatch && !anyCloseMatch)
            return null;
