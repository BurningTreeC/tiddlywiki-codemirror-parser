 ...
 * Wiki rule info type
 */
export interface WikiRuleInfo {
  name: string
  types: string
}

// Fallback core wiki parser rules (used when no callback provided)
export const coreWikiRules: WikiRuleInfo[] = [
  // Pragma rules
 ...
 */
export function wikiruleCompletion(
  getWikiRules?: () => WikiRuleInfo[]
  ...
  return (context: CompletionContext): CompletionResult | null => {
    ...
    const options: Completion[] = rules.map(rule => ({
      label: rule.name,
      type: "constant",
      detail: rule.types,
    }))
