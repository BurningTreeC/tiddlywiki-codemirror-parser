 ...
 */
import { Language, defineLanguageFacet, languageDataProp, foldNodeProp, indentNodeProp, foldService, syntaxTree, LanguageDescription, ParseContext } from "@codemirror/language";
 ...
 */
function getLineIndent(context, pos) {
 ...
 */
function containerIndent(context) {
 ...
 */
function isPragmaDefinition(name) {
        ...
        indentNodeProp.add({
            Document: (context) => {
                ...
                if (cursorLine.number > 1) {
                    ...
                    for (let n = nodeAtPrevEnd; n; n = n.parent) {
                        if (n.name === "PragmaEnd") {
                            foundPragmaEnd = true;
...
export const inlineConditionalFold = foldService.of((state, lineStart, lineEnd) => {
    ...
    while (cursor.next()) {
        ...
        if (cursor.from > lineEnd)
            break;
        // Skip nodes that start before our line
        if (cursor.from < lineStart)
            continue;
 ...
 */
export function getCodeParser(languages, defaultLanguage, tiddlywikiParser) {
    ...
    return (info) => {
        if (info && languages) {
            ...
            if (found instanceof LanguageDescription) {
                ...
                const langAlias = found.alias?.map((a) => a.toLowerCase()) || [];
