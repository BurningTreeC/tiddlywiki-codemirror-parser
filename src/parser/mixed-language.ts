...
export function extensionToLanguage(
  ext: string,
  codeLanguages: readonly LanguageDescription[] | undefined
): string {
  ...
  if (codeLanguages && Array.isArray(codeLanguages)) {
    for (const lang of codeLanguages) {
      ...
      if (lang.extensions && lang.extensions.some((e: string) => e.toLowerCase() === extNoDot)) {
 ...
 */
export function createMixedLanguageWrapper(
  ...
  return parseMixed((node: SyntaxNodeRef, input: Input) => {
    ...
    if (node.name === "HighlightStyles") {
      const cssParser = getParser("css") as { configure?: (options: { top: string }) => Parser } | null
