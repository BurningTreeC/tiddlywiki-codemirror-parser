...
import { LanguageSupport, syntaxHighlighting, indentOnInput, syntaxTree } from "@codemirror/language";
...
import { selfClosingTags, selfClosingWidgets, triggerCompletionOnAccept, widgetCompletion, widgetAttributeCompletion, attributeValueCompletion, wikitextAttributeCompletion, macroCompletion, macroParamCompletion, macroParamValueCompletion, substitutedParamCompletion, tiddlerCompletion, systemTiddlerCompletion, transclusionFieldCompletion, htmlTagCompletion, htmlAttributeCompletion, filterOperatorCompletion, filterRunPrefixCompletion, filterOperatorSuffixCompletion, filterOperandValueCompletion, conditionalCompletion, pragmaCompletion, pragmaEndNameCompletion, rulesKeywordCompletion, whitespaceValueCompletion, parsermodeValueCompletion, wikiruleCompletion, generalContextCompletion, fencedCodeCompletion, typedBlockCompletion, styledSpanClassCompletion, styledSpanPropertyCompletion, closingTagCompletion, } from "./completions";
...
export function tiddlywiki(config = {}) {
    const { codeLanguages, defaultCodeLanguage, addKeymap = true, base: { parser } = tiddlywikiLanguage, completeHTMLTags = true, completeWidgets = true, completeMacros = true, completeTiddlers = true, completeFilterOperators = true, completeFilterRunPrefixes = true, getTiddlerTitles, isDraftTiddler, getImageTiddlerTitles, getMacroNames, getMacroParams, getWidgetNames, getWidgetAttributes, getFilterOperators, getFieldNames, getTagNames, getTypeNames, getFileExtensions, getFunctionNames, getVariableNames, getTiddlerIndexes, getTiddlerFields, getStoryViews, getDeserializers, getPageClasses, getCSSProperties, getCSSValues, getCSSValuesForProperty, 
    ...
    htmlTagLanguage = htmlNoMatch, getSelfClosingWidgets, getWikiRules, disableCamelCaseLinks = false, enableKaTeX = false, getTabOutsideListBehavior, getShiftTabOutsideListBehavior, getEnterIndentBehavior, getFilterBracketMode, 
    ...
    nestedLanguageExtensions, nestedLanguageCompletionSources, nestedLanguageCompletions, } = config;
    // Validate parser
    ...
    let nestedLanguageCompletionOverride = null;
    ...
    if (nestedLanguageCompletions && nestedLanguageCompletions.length > 0) {
        ...
        nestedLanguageCompletionOverride = (context) => {
    ...
    const hasOptions = (result) => result?.options?.length > 0;
    const completionOverride = (context) => {
        ...
        if (hasOptions(styledClassResult))
            return styledClassResult;
        const styledPropResult = styledSpanPropertySource(context);
        if (hasOptions(styledPropResult))
            return styledPropResult;
    ...
    const wrap = createMixedLanguageWrapper(codeLanguages, defaultCode, parser, // Pass the TiddlyWiki parser for nested wikitext in typed blocks
    ...
    if (completeWidgets) {
        ...
        support.push(EditorView.inputHandler.of((view, from, _to, text) => {
            ...
            if (/<\/(\$[a-zA-Z0-9\-\$\.]*|[a-zA-Z][a-zA-Z0-9\-]*)\s*$/.test(textBefore))
                return false;
            ...
            if (isWidget) {
                ...
                if (allSelfClosingWidgets.has(tagName))
                    return false;
            }
            else {
            ...
            const scanForTags = () => {
                ...
                while (pos < len) {
                    ...
                    if (ch === '`' && textAfter[pos + 1] === '`' && textAfter[pos + 2] === '`') {
                        pos += 3;
                        while (pos < len && !(textAfter[pos] === '`' && textAfter[pos + 1] === '`' && textAfter[pos + 2] === '`'))
                            pos++;
                        pos += 3;
                        continue;
                    }
                    ...
                    if (ch === '<' && textAfter[pos + 1] === '<') {
                        ...
                        while (pos < len && macroDepth > 0) {
                            if (textAfter[pos] === '<' && textAfter[pos + 1] === '<') {
                                macroDepth++;
                                pos += 2;
                            }
                            else if (textAfter[pos] === '>' && textAfter[pos + 1] === '>') {
                                macroDepth--;
                                pos += 2;
                            }
                            else
                    ...
                    if (ch === '{' && textAfter[pos + 1] === '{') {
                        pos += 2;
                        while (pos < len && !(textAfter[pos] === '}' && textAfter[pos + 1] === '}'))
                            pos++;
                        pos += 2;
                        continue;
                    }
                    ...
                    if (ch === '<' && textAfter[pos + 1] === '/') {
                        ...
                        // Skip whitespace
                        while (pos < len && /\s/.test(textAfter[pos]))
                            pos++;
                        // Check for >
    ...
    if (completeFilterOperators) {
        support.push(lang.data.of({
            autocomplete: filterOperandValueCompletion(getTiddlerTitles, getTagNames, getFieldNames, getFunctionNames, getVariableNames, getTypeNames)
        }));
    }
    ...
    support.push(lang.data.of({
        autocomplete: generalContextCompletion(getTiddlerTitles, getMacroNames, getWidgetNames, getFunctionNames, getVariableNames, isDraftTiddler)
    }));
