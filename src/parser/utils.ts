 ...
 */
export function skipBracedBlock(text: string, pos: number): number {
 ...
 */
export function createFilterTextRef(content: string, refStart: number, refEnd: number): Element {
 ...
 */
export function createFilterVariable(content: string, varStart: number, varEnd: number): Element {
 ...
 */
export function createFilterMultiVariable(content: string, varStart: number, varEnd: number): Element {
  const contentStart = varStart + 1  // After (
 ...
 */
export function createAttributeNameElement(name: string, start: number, end: number): Element {
 ...
 */
export function createImageSourceElement(source: string, start: number, end: number): Element {
 ...
 */
export function createURLLinkElement(url: string, start: number, end: number): Element {
 ...
 */
export function parseFilterOperandPlaceholders(content: string, offset: number): Element[] {
...
export function parseTransclusionTarget(target: string, offset: number): Element[] {
  ...
  const parseTiddlerPart = (tiddler: string, partOffset: number) => {
...
export function parseFilterExpression(
  ...
  while (pos < content.length) {
    ...
    if (ch === '+' || ch === '-' || ch === '~' || ch === '=') {
      pos++
      // => shortcut for :let
      if (ch === '=' && pos < content.length && content[pos] === '>') pos++
      continue
    }
    if (ch === ':') {
 ...
 */
function findMatchingBracket(text: string, pos: number, open: string, close: string, skipBraces?: boolean): number {
  let depth = 0
  while (pos < text.length) {
    if (skipBraces) {
...
function createMacroParamNameElement(name: string, start: number, end: number): Element {
 ...
 */
function parseMacroParamValuePlaceholders(content: string, offset: number): Element[] {
  const elements: Element[] = []
  ...
  while (pos < len && iterations < maxIterations) {
    ...
    if (isNamedParam) {
      ...
      if (separatorChar === '=' && paramsStr.slice(pos, pos + 3) === '{{{') {
        ...
        const paramChildren: Element[] = [paramNameElement, valueElement]
      ...
      const paramChildren: Element[] = [valueElement]
