...
import { insertNewlineContinueMarkup, deleteMarkupBackward, toggleBold, toggleItalic, toggleUnderline, toggleStrikethrough, toggleSuperscript, toggleSubscript, toggleInlineCode, insertWikiLink, insertTransclusion, setHeading1, setHeading2, setHeading3, setHeading4, setHeading5, setHeading6, removeHeading, toggleBulletList, toggleNumberedList, insertCodeBlock } from "./commands";
// ============================================================================
    ...
    return getCached("tiddlerTitles", () => {
        try {
            ...
            if (wiki.eachShadowPlusTiddlers) {
                wiki.eachShadowPlusTiddlers((_tiddler, title) => {
...
function getTiddlerFields(tiddlerTitle) {
    if (typeof $tw === "undefined" || !tiddlerTitle)
        return [];
    try {
        const wiki = _currentEngine?.widget?.wiki || $tw.wiki;
        if (!wiki)
    ...
    try {
        ...
        if (widget?.getVariableInfo) {
            ...
            if (info && info.params && Array.isArray(info.params)) {
                return info.params.map((p) => p.name);
            }
        }
        return null;
    }
    ...
    return getCached("wikiRules", () => {
        try {
            ...
            for (const moduleName of Object.keys(wikiruleModules)) {
                ...
                const typeNames = [];
                if (module.types) {
                    if (module.types.pragma)
                        typeNames.push("pragma");
                    if (module.types.block)
                        typeNames.push("block");
                    if (module.types.inline)
                        typeNames.push("inline");
                }
                rules.push({
                    name: module.name,
...
function isKaTeXEnabled() {
    ...
    try {
        ...
        if ($tw.modules && $tw.modules.forEachModuleOfType) {
            let found = false;
            $tw.modules.forEachModuleOfType("widget", (_title, mod) => {
 ...
 */
function buildLanguageSupport(context, skipNestedLanguageExtensions = false) {
...
export const plugin = {
    ...
    getExtensions(context) {
        ...
        const langSupport = buildLanguageSupport(context, /* skipNestedLanguageExtensions */ false);
